...
const DEFAULT_TICKERS = "GOOGL, TSLA, MSFT, AAPL, NVDA, AMZN, META";
...
export default function USAnalysisPage() {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (res.success && res.signals) {
        ...
        setHasResults(true);
        toast.success(`Analysis complete — ${res.signal_count} signals generated`);
      } else {
  ...
  const pieData = Object.entries(decisionCounts).map(([name, value]) => ({
    ...
    value,
    color: DECISION_COLORS[name] || "#666",
  }));
  ...
  const holdCount = signals.filter((s) => s.decision?.toUpperCase().includes("HOLD")).length;
  ...
  const columns: ColumnDef<TradingSignal>[] = [
    {
      ...
      cell: ({ row }) => <span className="font-mono font-bold">{row.getValue("ticker")}</span>,
    },
    {
      accessorKey: "decision",
      header: ({ column }) => <SortableHeader column={column} title="Decision" />,
      cell: ({ row }) => {
        const d = row.getValue("decision") as string;
        return <Badge className={getDecisionBadgeClass(d)}>{d}</Badge>;
      },
    },
    {
      ...
      cell: ({ row }) => (
        <span className={getDecisionColor(row.original.decision)}>
          {formatNumber(row.getValue("decision_score") as number, 3)}
        </span>
      ),
    },
    {
      ...
      header: "Price",
      cell: ({ row }) => `$${formatNumber(row.getValue("current_price") as number)}`,
    },
    {
      accessorKey: "rsi",
      header: "RSI",
      cell: ({ row }) => formatNumber(row.getValue("rsi") as number, 1),
    },
    {
      accessorKey: "sentiment_label",
      header: "Sentiment",
      cell: ({ row }) => {
        const label = row.getValue("sentiment_label") as string;
        return label ? (
    ...
    },
    {
      ...
      header: "Reasoning",
      cell: ({ row }) => (
        <span className="max-w-xs truncate text-muted-foreground" title={row.getValue("reasoning") as string}>
          {(row.getValue("reasoning") as string)?.slice(0, 60)}…
        </span>
      ),
    },
  ];
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold">US Stock Analysis</h1>
        <p className="text-sm text-muted-foreground">
          News scraping, sentiment analysis, fundamental & technical metrics, trading signals
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle className="text-base">Ticker Selection</CardTitle>
          </CardHeader>
            ...
            {inputMode === "manual" ? (
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  placeholder="GOOGL, TSLA, MSFT..."
                  rows={3}
                />
                <p className="text-xs text-muted-foreground">
                  {parseTickers().length} ticker(s) selected
                </p>
              </div>
            ) : (
              <div className="space-y-2">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <Button
              onClick={handleRun}
              disabled={loading || parseTickers().length === 0}
              className="w-full"
              size="lg"
            >
              {loading ? (
      ...
      {hasResults && (
        <>
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            ...
            <MetricCard
              title="Avg Score"
              value={formatNumber(avgScore, 3)}
              changeType={avgScore > 0 ? "positive" : avgScore < 0 ? "negative" : "neutral"}
          ...
          <Tabs defaultValue="overview" className="space-y-4">
            ...
            <TabsContent value="top">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="text-base text-profit">Top Buy Signals</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    {signals
                      .filter((s) => s.decision?.toUpperCase().includes("BUY"))
                      .sort((a, b) => b.decision_score - a.decision_score)
                      .slice(0, 5)
                      .map((s) => (
                        <div key={`${s.ticker}-${s.source}`} className="flex items-center justify-between rounded-lg border p-3">
                          <div>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="text-base text-loss">Top Sell Signals</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    {signals
                      .filter((s) => s.decision?.toUpperCase().includes("SELL"))
