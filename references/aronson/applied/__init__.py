"""
aronson.applied — chapter registry, async batch runner, and progress tracking.

Aronson Evidence-Based Technical Analysis — Statistical Validation Lab.
"""
...
_CHAPTERS = [
    # Signal Testing
    {"key": "ch01", "title": "Detrended Returns & t-Stat Gating",
     "category": "Signal Testing",
     "description": "Zero-centre returns, per-signal t-stat gate (t≥2 ↔ p<0.05)"},
    {"key": "ch02", "title": "Benjamini-Hochberg FDR",
     "category": "Signal Testing",
     "description": "Control false discovery rate across N signals tested simultaneously"},
    # Multi-Test Correction
    {"key": "ch03", "title": "White's Reality Check",
     "category": "Multi-Test Correction",
     "description": "Bootstrap null for best-of-N bias — is the best signal real?"},
    {"key": "ch04", "title": "Data-Mining Bias Estimation",
     "category": "Multi-Test Correction",
     "description": "Estimate expected inflation σ√(2·ln N) from multiple testing"},
    # Composite Validation
    {"key": "ch05", "title": "Full Aronson Validation Pipeline",
     "category": "Composite Validation",
     "description": "End-to-end: detrend → t-stat → BH-FDR → WRC → confidence score"},
]
...
def _execute_chapter(
    ...
    env_overrides = {}
    if tickers:
        env_overrides["ARON_TICKERS"] = ",".join(tickers)
    if date_start:
        env_overrides["ARON_DATE_START"] = date_start
    if date_end:
        env_overrides["ARON_DATE_END"] = date_end

    old_env = {k: os.environ.get(k) for k in env_overrides}
    for k, v in env_overrides.items():
        os.environ[k] = v

    aron_dir = str(_APPLIED_DIR.parent)
    if aron_dir in sys.path:
        sys.path.remove(aron_dir)
    sys.path.insert(0, aron_dir)
    ...
    try:
        ...
        logger.exception("Aronson chapter %s failed", ch_key)
    finally:
        if aron_dir in sys.path:
            sys.path.remove(aron_dir)
...
async def run_chapters_async(
    ...
    try:
        ...
        await asyncio.to_thread(
            save_batch_history, batch_id, "aronson", chapter_keys, snapshot,
            tickers, date_start, date_end,
        )
    except Exception:
        logger.warning("Failed to save Aronson batch history", exc_info=True)
